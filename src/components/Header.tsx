...
import { CalculationMethodType, LocationData, AdminVisibilityConfig } from '../types';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const primaryNavItems = allPrimaryNav.filter(
    (item) => !adminConfig || adminConfig.enabledTabs[item.id as keyof AdminVisibilityConfig['enabledTabs']] !== false
  );

  const secondaryNavItems = allSecondaryNav.filter(
    (item) => !adminConfig || adminConfig.enabledTabs[item.id as keyof AdminVisibilityConfig['enabledTabs']] !== false
  );

  const showSearch = adminConfig ? adminConfig.showHeaderSearch : true;
  const showLocation = adminConfig ? adminConfig.showHeaderLocation : true;
  const showMethod = adminConfig ? adminConfig.showHeaderCalculationMethod : true;
  const showTheme = adminConfig ? adminConfig.showHeaderThemeToggle : true;
  const showAdmin = adminConfig ? adminConfig.showHeaderAdminButton : true;
        ...
        <div className="flex items-center justify-between h-16 sm:h-20 gap-2 sm:gap-4">
          ...
          >
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-red-600 via-rose-600 to-amber-600 p-0.5 shadow-md shadow-red-500/20 group-hover:scale-105 transition-transform flex items-center justify-center">
              <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center p-1 text-xl sm:text-2xl">
                {adminConfig?.companyProfile?.customLogoEmoji ? (
                  <span>{adminConfig.companyProfile.customLogoEmoji}</span>
                ) : (
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <h1 className="text-lg sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-1 font-['Mukta',sans-serif]">
                  {adminConfig?.companyProfile?.companyNameNepali || (
                    <>
                      नाम्रो <span className="text-red-600 dark:text-rose-500">स्मार्ट</span> पात्रो
                    </>
                  )}
                </h1>
                {adminConfig?.companyProfile?.displayOrganizationBadge && (
                  <span className="hidden sm:inline-flex px-1.5 py-0.2 rounded-full text-[9px] font-black bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-700">
                    ✓ प्रमाणित
                  </span>
                )}
                {adminConfig?.role === 'admin' && (
                  <span className="hidden md:inline-flex px-2 py-0.5 text-[10px] font-black bg-red-100 dark:bg-red-950/80 text-red-700 dark:text-rose-300 rounded-full border border-red-200 dark:border-red-800/60">
                    👑 ADMIN
                  </span>
                )}
                {adminConfig?.role === 'kiosk' && (
                  <span className="hidden md:inline-flex px-2 py-0.5 text-[10px] font-black bg-purple-100 dark:bg-purple-950/80 text-purple-700 dark:text-purple-300 rounded-full border border-purple-200 dark:border-purple-800/60">
                    📺 KIOSK TV
                  </span>
                )}
              </div>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium hidden sm:block truncate max-w-sm">
                {adminConfig?.companyProfile?.taglineNepali || 'स्वतन्त्र नेपाली पात्रो, पञ्चाङ्ग, रेडियो तथा समाचार पोर्टल'}
              </p>
            </div>
          </div>
          ...
          {primaryNavItems.length > 0 && (
            <div className="hidden lg:flex items-center gap-1 bg-slate-100/80 dark:bg-slate-800/80 p-1 rounded-2xl border border-slate-200/80 dark:border-slate-700/60">
              ...
              {secondaryNavItems.length > 0 && (
                <div className="relative">
                  ...
                  >
                    <span>थप ({secondaryNavItems.length})</span>
                    <ChevronDown className="w-3 h-3" />
                  </button>

                  {showMoreMenu && (
                    <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-1.5 z-50 animate-in fade-in zoom-in-95">
                      {secondaryNavItems.map((item) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 text-xs font-bold rounded-xl flex items-center gap-2 transition-colors ${
                            activeTab === item.id
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {/* Admin Control Center Suite Button */}
            {showAdmin && (
              <button
                id="btn-open-admin-suite"
                onClick={onOpenAdminControl || onOpenAdminLab}
                className="px-2.5 py-1.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-bold text-xs shadow-md shadow-red-500/20 flex items-center gap-1.5 transition-transform active:scale-95"
                title="व्यवस्थापक नियन्त्रण केन्द्र (Admin Control & Feature Switcher)"
              >
